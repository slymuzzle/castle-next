...
func generateShort(length int) (string, error) {
	if length > 7 || length <= 0 {
		return "", fmt.Errorf("invalid length %d. Try to use Gen() method instead", length)
	}
...
func generateArrayShort(length int, amount int, offset int) ([]int, error) {
	...
	for i := range arr {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid length %d. Try to use GenerateArray() method instead", length)
		}
